...
int beneficio_estimado_camion(vector<int> const& maxCalidad, Nodo const& X, int N) {
	if (X.k == N - 1) return X.beneficio;
	return X.beneficio + maxCalidad[X.k + 1];
}

void funcionarios_rp_camion(vector<vector<int>> const& calidades, vector<int> const& anchuraCarros, vector<int> const& anchuraCaminos, vector<int> const& maxCalidad, vector<int>& sol_mejor, int& beneficio_mejor, int N, int M) {
	// se genera la raíz
	Nodo Y;
